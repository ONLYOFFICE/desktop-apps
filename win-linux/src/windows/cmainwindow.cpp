...
//    mainPanel->setProperty("uithemetype", AscAppManager::themes().current().stype());
    ...
    // Set custom TabBar
    CTabBar *m_pTabBar = new CTabBar(mainPanel);
    _pMainGridLayout->addWidget(m_pTabBar, 0, 1, 1, 1);
    QSizePolicy sizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
    sizePolicy.setHorizontalStretch(1);
    m_pTabBar->setSizePolicy(sizePolicy);
...
//    m_pTabs->setAutoFillBackground(true);
    // Set TabWidget
    m_pTabs = new CAscTabWidget(mainPanel, m_pTabBar);
