...
static double GetLogicalDpi(const QPoint &pt)
{
#ifdef __OS_WIN_XP
    HDC hdc = GetDC(NULL);
    ...
    m_allowMaximize(true)
{
    ...
    double dpi = GetLogicalDpi(geometry().center());
    GetFrameMetricsForDpi(m_frame, dpi, m_isMaximized);
}
...
bool CWindowPlatform::nativeEvent(const QByteArray &eventType, void *message, long *result)
{
    ...
    case WM_DPICHANGED: {
        setMinimumSize(0,0);
        GetFrameMetricsForDpi(m_frame, (double)HIWORD(msg->wParam)/96, m_isMaximized);
            ...
            if (!monitor) {
                ...
                if (GetMonitorInfo(monitor, &monInfo)) {
                    ...
                    if (EnumDisplayDevices(monInfo.szDevice, 0, &dispDevice, EDD_GET_DEVICE_INTERFACE_NAME)) {
                        HANDLE hDevice = CreateFile(dispDevice.DeviceID, GENERIC_READ, FILE_SHARE_READ,
...
//            ::DeleteObject(hrgn);
    ...
    case WM_GETMINMAXINFO: {
        ...
        if (m_isMaximized != isMaximized) {
            ...
            double dpi = GetLogicalDpi(geometry().center());
            GetFrameMetricsForDpi(m_frame, dpi, isMaximized);
        }
        break;
    }
