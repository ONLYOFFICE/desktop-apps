...
bool SetAssocLegacy(const char *ext, const char *progID, const char *subkey)
{
    // TODO: add impl
    return false;
}
...
bool SetUserFileAssoc(const wchar_t* ext, const wchar_t* progId)
{
    ...
    if (winVer > Win7) {
        ...
        if (!SetAssocLegacy(lpExt, lpProgId, lpSubkey)) {
            return false;
        }
    }
    return true;
...
bool SetUserFileAssoc(const AssocPair* assocArray, size_t count)
{
    for (int i = 0; i < count; ++i) {
        if (!SetUserFileAssoc(assocArray[i].extension, assocArray[i].progId))
            return false;
    }
    return true;
}
